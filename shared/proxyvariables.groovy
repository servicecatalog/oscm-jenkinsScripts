    ...
    def _setProxyVariables = {
        ...
        stage('Build - set proxy variables') {
	        if ( "${http_proxy}" != ''  && "${https_proxy}" != '') {
                ...
                env.MAVEN_OPTS="-Xmx512m -Duser.home=/build -Dhttp.proxyHost=httpUri?.getHost() -Dhttp.proxyPort=httpUri?.getPort()"
            } else if ( "${https_proxy}" != '' ) {
                URI httpsUri = new URI("${https_proxy}") 
                env.ANT_OPTS="-Dhttps.proxyHost=httpsUri?.getHost() -Dhttps.proxyPort=httspUri?.getPort()"
                env.MAVEN_OPTS="-Xmx512m -Duser.home=/build -Dhttps.proxyHost=httpsUri?.getHost() -Dhttps.proxyPort=httspUri?.getPort()"
           } else {
                env.ANT_OPTS=""
